...
export class TrainingsTableDataSource implements DataSource<Exercice> {
	...
	private exercices = new BehaviorSubject<Exercice[]>([]);

	constructor(private exerciceService: ExerciceService) {}

	retrieveExercice(): void  {
		this.exerciceService.getRealtimeExerciceRef().valueChanges().subscribe(exercicesDB => {
			this.exercices.next(exercicesDB);
		...
		return data.sort((a, b) => {
			const isAsc = this.sort?.direction === 'asc';
				switch (this.sort?.active) {
					case 'exercice': return compare(a.name, b.name, isAsc);
